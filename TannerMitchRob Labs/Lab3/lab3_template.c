...
//#warning "Unimplemented function: void move_stepper_motor_by_step(int num_steps, int direction)"	// delete this line after implementing
    ...
    int i = 0;
    static unsigned char prev = 0b00010000; //holds last position of motor
    GPIO_PORTB_DATA_R = prev;

    //Clockwise movement
    if(direction == 1){

        //while loop that runs until you get to number of steps
        while(i < num_steps){

        //if loop resets bits to 0b00010000 if shifter gets to 0b10000000
        if((GPIO_PORTB_DATA_R & 0b10000000) == 0b10000000)
            GPIO_PORTB_DATA_R = 0b00010000;

        //delay, then shifts to left
        timer_waitMillis(50);
        GPIO_PORTB_DATA_R = GPIO_PORTB_DATA_R << 1;

        //if shaft encoder has moved an addition spot, we subtract 10 from i (this is for the people who turn the shaft encoder a little too quickly)
        if(read_shaft_encoder() > 0)
            i = i-10;
        i++;
        }
    }
    ...
    if(direction == -1){
            ...
            //while loop that runs until you get to number of steps
            while(i < num_steps){
            //if loop resets the bits to 0b10000000 if it gets to 0b00010000
            if((GPIO_PORTB_DATA_R & 0b00010000) == 0b00010000)
                GPIO_PORTB_DATA_R = 0b10000000;

            //delay, then shifts bits to right
            timer_waitMillis(50);
            GPIO_PORTB_DATA_R = GPIO_PORTB_DATA_R >> 1;
    ...
    prev = (GPIO_PORTB_DATA_R & 0b11110000);
...
int main(void) {
	...
	//just reads in the buttons, and the shaft, and inputs into the move_stepper_motor_by_step function and prints the shaft/buttons from step1/2 on screen
	int sum = 0, shaft = 0;
	uint8_t button;
	while(1){
	    button = button_getButton();
	    shaft = read_shaft_encoder();
		sum += shaft;
